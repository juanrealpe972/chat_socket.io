    ...
    return (
        <div className="h-screen bg-zinc-800 text-white flex items-center justify-center">
            <form
                ...
                className="bg-zinc-900 p-10 flex flex-col w-2/4"
            >
                <h1 className="text-2xl text-center font-bold my-2">Chat</h1><br />
                ...
                <ul className="overflow-y-auto h-64 scroll-smooth">
                    {messages.map((message, i) => (
                        <li
                            key={i}
                            className={`my-2 p-2 table text-sm rounded-md ${message.from === "Me" ? "bg-cyan-700" : "bg-green-700 ml-auto"}`}
                        >
                            <span className={`text-xs font-mono ${ message.from === "Me" ? "text-orange-500" : "text-red-500" }`} >
                                {message.from}
                            </span>
